...
class SlurmNode(metaclass=ABCMeta):
    ...
    def needs_reset_when_inactive(self):
        """Check if the node need to be reset in node is inactive."""
        pass

    def get_instance_name(self):
        """Get instance name of given node."""
        nodename_capture = re.match(r"^([a-z0-9\-]+)-(st|dy)-([a-z0-9]+)-\d+$", self.name)
        if not nodename_capture:
            raise InvalidNodenameError
        _, _, instance_name = nodename_capture.groups()
...
class StaticNode(SlurmNode):
    ...
    def needs_reset_when_inactive(self):
        """Check if the node need to be reset iif node is inactive."""
