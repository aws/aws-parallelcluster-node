...
def addHost(hostname,cluster_user,slots):
    ...
    command = ("/opt/pbs/bin/qmgr -c 'create node %s np=%s'" % (hostname, slots))
    __runCommand(command)

    command = ('/opt/pbs/bin/pbsnodes -c %s' % hostname)
    __runCommand(command)
    ...
    command = ('/etc/init.d/pbs restart')
    __runCommand(command)

def removeHost(hostname, cluster_user):
    log.info('Removing %s', hostname)

    command = ('/opt/pbs/bin/pbsnodes -o %s' % hostname)
    __runCommand(command)

    command = ("/opt/pbs/bin/qmgr -c 'delete node %s'" % hostname)
    __runCommand(command)
